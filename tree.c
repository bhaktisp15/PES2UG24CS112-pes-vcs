...
// Recursively builds subtree objects bottom-up before parent
static int write_tree_level(IndexEntry *entries, int count, int depth, ObjectID *id_out) {
    ...
    while (i < count) {
        ...
        // Walk past 'depth' slashes to find the component at this level
        char *p = path;
        ...
        if (!slash) {
            // It's a file at this level — add as blob entry directly
            ...
            // It's a subdirectory — collect all entries with the same dir prefix
            size_t dir_len = slash - p;
            char dir_name[256];
            strncpy(dir_name, p, dir_len);
            dir_name[dir_len] = '\0';

            // Find how many entries share this subdirectory
            int j = i;
            while (j < count) {
                ...
                if (!sj) break;
                size_t dj_len = sj - pj;
                if (dj_len != dir_len || strncmp(pj, dir_name, dir_len) != 0) break;
                j++;
            }
